...
export default function AdminOrdersPage() {
  ...
  const getStatusLabel = (status: string, deliveryMode?: string, address?: string) => {
    const isPickup = deliveryMode === 'PICKUP' || address?.includes('Atelier LUXE');
    if (isPickup) {
    ...
    <div className="admin">
      <div className="admin__header">
        <div className="admin__header-left">
          <Link to="/" className="admin__back"><FiArrowLeft size={18} /> Volver a Boutique</Link>
          <h1 style={{ fontFamily: 'var(--font-serif)', letterSpacing: '1px' }}>
            <FiPackage size={28} style={{ marginRight: '12px' }} /> 
            ADMIN <span>STUDIO LUXE</span>
          </h1>
        </div>
        <Link to="/admin/productos" className="btn btn--outline">Gestionar Catálogo</Link>
      </div>
