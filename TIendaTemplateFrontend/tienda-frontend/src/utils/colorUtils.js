...
/** Si el label tiene formato "#rrggbb / Talla" o "#rrggbb - Talla", reemplaza el hex por el nombre */
export function formatVariantLabel(label) {
  if (!label) return label;
  return label.replace(/^(#[0-9a-fA-F]{6})\s*([\/\-])\s*/, (_, hex, sep) => `${hexToColorName(hex)} ${sep} `);
}
